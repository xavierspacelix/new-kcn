...
const DialogDaftarUangMuka: React.FC<DialogDaftarUangMukaProps> = ({ visible, stateDataHeader, setStateDataHeader, dataDaftarUangMuka, swalToast, formatDate, CurrencyFormat }) => {
  ...
  let gridDaftarUangMuka = useRef<GridComponent>(null);
  ...
  buttonDaftarUangMuka = [
    {
      ...
      isFlat: false,
      click: () => {
        currentDaftarUangMuka = gridDaftarUangMuka.current!.getSelectedRecords();
  ...
  return (
    <DialogComponent
      ...
      close={() => {
        gridDaftarUangMuka.current!.clearSelection();
        setStateDataHeader((prevState: any) => ({
          ...prevState,
          dialogDaftarUangMukaVisible: false,
        }));
    ...
    >
      ...
      <GridComponent
        ...
        recordDoubleClick={(args: any) => {
          if (gridDaftarUangMuka.current!.dataSource) {
            const rowIndex: number = args.row.rowIndex;
            gridDaftarUangMuka.current!.selectRow(rowIndex);
            const currentDaftarUangMuka = gridDaftarUangMuka.current!.getSelectedRecords();
            if (currentDaftarUangMuka.length > 0) {
              handleClickDaftarUangMuka(currentDaftarUangMuka);
              setStateDataHeader((prevState: any) => ({
                ...prevState,
