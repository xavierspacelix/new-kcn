...
import { frmNumber, generateNU, FillFromSQL, fetchPreferensi } from '@/utils/routines';
...
const DialogBMPOSList: React.FC<DialogBMPOSList> = ({
            ...
            if (data.status) {
                const formattedData = data.data.alokasi.map((item: any, index: number) => {
                    ...
                    return {
                        ...
                        tgl_fj: tglFj,
                        tgl_trxfj: tglTrxFj,
                        sisa: parseInt(item.owing),
                        netto_mu: parseInt(item.netto_mu),
                        total_pajak_rp: parseInt(item.total_pajak_rp),
                        owing: parseInt(item.owing),
                        hari: hari,
                    };
                });
    ...
    const validate = () => {
        // console.log(gridAlokasiDana.dataSource, 'grid');
        // console.log(gridAlokasiFakturPOS.dataSource, 'grid');
    ...
    const saveDoc = async () => {
        // const modifiedDetailJurnal: any = gridAlokasiDana.dataSource.map((item: any, index: number) => {
        ...
        const reqBody = {
            ...
            no_warkat: null,
            tgl_valuta: null,
            kode_cust: selectedSubledgerAllDataHeader.kode_subledger,
        ...
        try {
            if (statusPage === 'CREATE') {
                var responseAPI = await axios.post(`${apiUrl}/erp/simpan_bm`, reqBody, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                });
            } else {
                var responseAPI = await axios.patch(`${apiUrl}/erp/update_pemasukkan_lain_bm`, reqBody, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                });
            ...
            if (responseAPI.data.status === true) {
                if (statusPage === 'CREATE') {
                    await generateNU(kode_entitas, statusPage === 'EDIT' ? mNoBM : noBm, '17', moment().format('YYYYMM'));
    ...
    const kalkulasiJurnal = () => {
        ...
        if (inputValueJumlah_mu) {
            newTotalDebet += Number(formatNumber(inputValueJumlah_mu));
        }
    ...
    const resultSelisih = Number(formatNumber(inputValueJumlah_mu)) - totalKredit;
    ...
    const checkboxTemplate = (props: any) => <input type="checkbox" checked={props.pay} onChange={() => handleCheckboxChange(props)} />;
