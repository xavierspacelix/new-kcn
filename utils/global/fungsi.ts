  ...
  try {
    ...
    if (iKELAS === '' || iKELAS === 'G' || iKELAS === 'L' || iKELAS === 'M' || iKELAS === 'N') {
      ...
      if (responseDataPref[0].hari_blacklist > 0 && (iODFAKTUR > responseDataPref[0].hari_blacklist || iODWARKAT > responseDataPref[0].hari_blacklist)) {
        swal.fire({
          title: 'Terdapat faktur atau warkat tagihan melebihi ' + +responseDataPref[0].hari_blacklist + ' hari dari tgl. terima' + 'Kebijakan perusahaan customer diklasifikasikan "G" (Blacklist).',
          icon: 'warning',
        });
